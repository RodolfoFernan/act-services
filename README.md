...
CREATE OR REPLACE PROCEDURE FES.FESSPZ24_CRISE2019_ALTERA_LIB
--C077033  26/05/2022 16:29:47
--C077033  24/05/2022 14:00:00
--C077033  24/01/2022 17:12:00
--C077033  20/10/2021 18:00:00
--C077033  29/10/2020 14:42:00
--C077033  30/09/2020 11:03:00
--C077033  17/08/2020 13:00:00 - INSERE RETENCAO POR AUSENCIA DE ADITAMENTO VALIDO
--C077033  29/05/2020 10:00:00 - CONSIDERAR SOMENTE STATUS VALIDOS P/ TRANSFERENCIA/SUSPENSAO
--F620600  18/02/2020 11:54:42
--F932471  08/12/2019 21:35:16
--F620600  06/12/2019 23:16:00
--C101492  08/12/2019 21:33:00 - EXCLUSAO DE PARAMETRO P/ EXECUCAO EM PRD.

AS
		SQL_QUERY VARCHAR2(500) := NULL;
		COUNT_TABELA NUMERIC(5) := 0;
BEGIN
	DBMS_OUTPUT.PUT_LINE('INICIO EXECUCAO FESSPZ24_CRISE2019_ALTERA_LIB');

	-- Cursor contendo todos os candidatos que possuem: 1) liberacoes com situação NR e possuem transferencia ou suspensao;
	-- 2) Liberacoes a estornar;
	FOR x IN
	(
		SELECT
		...
		WHERE
		(
			L.IC_SITUACAO_LIBERACAO = 'NR'		-- NAO REPASSADO C/ TRASNFERENCIA OU SUSPENSAO
			AND (
					EXISTS 	(
								SELECT 1
								FROM FES.FESTB049_TRANSFERENCIA T
								WHERE l.NU_SEQ_CANDIDATO = T.NU_CANDIDATO_FK10
					...
					EXISTS  (
								...
								WHERE O.IC_TIPO_OCORRENCIA = 'S'
								AND l.NU_SEQ_CANDIDATO = O.NU_CANDIDATO_FK36
								AND O.NU_STATUS_OCORRENCIA = 11
							)
				)
		)
		OR (L.IC_SITUACAO_LIBERACAO = 'NE')
	)
	LOOP
		-- SE FOR NE
		IF x.IC_SITUACAO_LIBERACAO = 'NE' THEN
			INSERT 
			INTO FES.FESTB817_RETENCAO_LIBERACAO
				 ( NU_SQNCL_LIBERACAO_CONTRATO, NU_MOTIVO_RETENCAO_LIBERACAO, DT_INICIO_RETENCAO )
			SELECT NU_SQNCL_LIBERACAO_CONTRATO, '5', SYSDATE
			FROM FES.FESTB712_LIBERACAO_CONTRATO T712
			WHERE T712.NU_SQNCL_LIBERACAO_CONTRATO = x.NU_SQNCL_LIBERACAO_CONTRATO
			AND NOT EXISTS (
								SELECT 1 
								FROM FES.FESTB817_RETENCAO_LIBERACAO T817
			   					WHERE T817.NU_SQNCL_LIBERACAO_CONTRATO = T712.NU_SQNCL_LIBERACAO_CONTRATO
			   					AND T817.NU_MOTIVO_RETENCAO_LIBERACAO = '5'
			   					AND T817.DT_FIM_RETENCAO IS NULL
							);
		END IF;

		-- SE POSSUI TRANSFERENCIA
		SELECT COUNT(1) INTO COUNT_TABELA
		FROM FES.FESTB049_TRANSFERENCIA
		WHERE NU_CANDIDATO_FK10 = x.NU_SEQ_CANDIDATO
		AND NU_STATUS_TRANSFERENCIA = 5;

		IF COUNT_TABELA > 0 THEN
			INSERT INTO FES.FESTB817_RETENCAO_LIBERACAO
		  	   (NU_SQNCL_LIBERACAO_CONTRATO, NU_MOTIVO_RETENCAO_LIBERACAO, DT_INICIO_RETENCAO)
			SELECT NU_SQNCL_LIBERACAO_CONTRATO, '2', SYSDATE
			FROM FES.FESTB712_LIBERACAO_CONTRATO T712
			WHERE T712.NU_SQNCL_LIBERACAO_CONTRATO = x.NU_SQNCL_LIBERACAO_CONTRATO
			AND NOT EXISTS 	(
								SELECT 1 
								FROM FES.FESTB817_RETENCAO_LIBERACAO T817
			   					WHERE T817.NU_SQNCL_LIBERACAO_CONTRATO = T712.NU_SQNCL_LIBERACAO_CONTRATO
			   					AND T817.NU_MOTIVO_RETENCAO_LIBERACAO = '2'
			   					AND T817.DT_FIM_RETENCAO IS NULL
							);
		END IF;
		...
		AND NU_CANDIDATO_FK36 = x.NU_SEQ_CANDIDATO
		AND NU_STATUS_OCORRENCIA = 11;

		IF COUNT_TABELA > 0 THEN
			INSERT INTO FES.FESTB817_RETENCAO_LIBERACAO
		  	   (NU_SQNCL_LIBERACAO_CONTRATO, NU_MOTIVO_RETENCAO_LIBERACAO, DT_INICIO_RETENCAO)
			SELECT NU_SQNCL_LIBERACAO_CONTRATO, '3', SYSDATE
			FROM FES.FESTB712_LIBERACAO_CONTRATO T712
			WHERE T712.NU_SQNCL_LIBERACAO_CONTRATO = x.NU_SQNCL_LIBERACAO_CONTRATO
			AND NOT EXISTS 	(
								SELECT 1 
								FROM FES.FESTB817_RETENCAO_LIBERACAO T817
			   					WHERE T817.NU_SQNCL_LIBERACAO_CONTRATO = T712.NU_SQNCL_LIBERACAO_CONTRATO
			   					AND T817.NU_MOTIVO_RETENCAO_LIBERACAO = '3'
			   					AND T817.DT_FIM_RETENCAO IS NULL
							);
		END IF;
    ...
    DBMS_OUTPUT.PUT_LINE(' ************* FIM DO PROCESSAMENTO DA INCLUSAO DE RETENCOES POR TRANSFERENCIA, SUSPENSAO E A ESTORNAR ************* ');

    -- SE O REGISTRO DE TROCA DE MANTENCA POSSUI MANTENEDORA OU IES NAO CADASTRADA NO SIFES INCLUI RETENCAO
	...
	(
        ...
        AND L.NU_SQNCL_LIBERACAO_CONTRATO NOT IN 	(
														SELECT T817.NU_SQNCL_LIBERACAO_CONTRATO
														FROM FES.FESTB817_RETENCAO_LIBERACAO T817
														WHERE T817.NU_MOTIVO_RETENCAO_LIBERACAO = '6'
														AND T817.DT_FIM_RETENCAO IS NULL
													)
		AND L.NU_IES IN
						(
							131, 554, 913, 1290, 1433, 1443, 1464, 1576, 1622, 1732,
							1745, 1973, 2827, 2832, 3311, 3675, 4053, 4123, 4357, 4584,
							4765, 5000, 12601, 12899, 13865, 16759, 17269, 17608, 18009,
							18036, 18255, 18258, 18497, 18509, 18696, 18719, 18958,
							19252, 19357, 19358, 19859, 21537, 21906,
							21993, 22462
						)
						-- RELACAO DE IES COM PROBLEMAS DE CADASTRO
						-- (MANTENEDORA OU IES NAO EXISTE NO SIFES)
    )
	LOOP
        SQL_QUERY := 'INSERT INTO FES.FESTB817_RETENCAO_LIBERACAO ' ||
			     ...
			     x.NU_SQNCL_LIBERACAO_CONTRATO || ', ''6'',''' || SYSDATE || ''')';
    ...
    DBMS_OUTPUT.PUT_LINE(' ************* FIM DO PROCESSAMENTO DA INCLUSAO DE RETENCOES POR PROBLEMA CADASTRAL (IES/MANTENEDORA NAO EXISTE) ************* ');


	-- CURSOR PARA SELECIONAR LIBERACOES SEM TRANSACAO VALIDA
	FOR X IN
	(
		SELECT L.NU_SQNCL_LIBERACAO_CONTRATO
		FROM FES.FESTB712_LIBERACAO_CONTRATO L
		...
		WHERE
				(
					...
					AND 
						(
							CA.NU_SEQ_CANDIDATO IS NULL 
							OR F.NU_CANDIDATO_FK11 IS NULL
						)
				)
		AND L.IC_SITUACAO_LIBERACAO IN ('R', 'NR')
		AND L.NU_SEQ_CANDIDATO > 20000000
		AND NOT EXISTS  (
							SELECT 1
	...
	LOOP
		--INSERE RETENCAO POR AUSENCIA DE ADITAMENTO VALIDO
		SQL_QUERY := 'INSERT INTO FES.FESTB817_RETENCAO_LIBERACAO ' ||
		  	    ' (NU_SQNCL_LIBERACAO_CONTRATO, NU_MOTIVO_RETENCAO_LIBERACAO, DT_INICIO_RETENCAO) values (' ||
			     x.NU_SQNCL_LIBERACAO_CONTRATO || ', ''7'',''' || SYSDATE || ''')';
	...
	DBMS_OUTPUT.PUT_LINE(' ************* FIM DO PROCESSAMENTO DA INCLUSAO DE RETENCAO POR AUSENCIA DE TRANSACAO VALIDA ************* ');


	-- CURSOR PARA SELECIONAR LIBERACOES SEM FINALIZACAO NO PROCESSO DE ADITAMENTO
	FOR X IN
	(
		SELECT
	...
	LOOP
		--INSERE RETENCAO POR AUSENCIA DE FINALIZACAO NO PROCESSO DE ADITAMENTO
		SQL_QUERY := 'INSERT INTO FES.FESTB817_RETENCAO_LIBERACAO ' ||
				' (NU_SQNCL_LIBERACAO_CONTRATO, NU_MOTIVO_RETENCAO_LIBERACAO, DT_INICIO_RETENCAO) values (' ||
				 X.NU_SQNCL_LIBERACAO_CONTRATO || ', ''10'',''' || SYSDATE || ''')';
	...
	DBMS_OUTPUT.PUT_LINE(' ************* FIM DO PROCESSAMENTO DA INCLUSAO DE RETENCAO POR AUSENCIA DE FINALIZACAO NO PROCESSO DE ADITAMENTO ************* ');

	DBMS_OUTPUT.PUT_LINE('FIM EXECUCAO FESSPZ24_CRISE2019_ALTERA_LIB');
